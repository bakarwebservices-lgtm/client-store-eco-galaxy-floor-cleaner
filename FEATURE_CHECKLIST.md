...
| **Reviews (Admin)** | Review CSV Import (`src/components/admin/ReviewCsvImportModal.tsx`) | Shopify/Judge.me/Loox format parser, SKU/slug product matcher, preview/validation modal, pending moderation batch commit | `POST /api/admin/reviews/import` | `Connected` |
...
| **Admin Layout** | Persistent Navigation Shell (`src/components/admin/AdminSidebar.tsx`) | 15-section persistent desktop sidebar + mobile drawer across all `/admin/*` pages with active route highlight and role badges | `src/app/(admin)/layout.tsx` | `Connected` |
| **Admin Operations** | Universal Bulk Operations (`src/components/admin/BulkActionBar.tsx`) | Reusable bulk select-all UI and transactional API endpoints across Orders, Products, Categories, Collections, Coupons, Reviews, Blog, Pages, FAQ, Newsletter, Media, Waitlist, and Contact messages | `POST /api/admin/*/bulk` | `Connected` |
| **Transactional Emails** | Order Confirmation Email (`src/lib/email/index.ts`) | HTML order receipt with snapshot items, address, subtotal/shipping/discount breakdown, dispatched on checkout | Dispatched on `POST /api/checkout` | `Connected` |
| **Transactional Emails** | Fulfillment Status Email (`src/lib/email/index.ts`) | Dispatched when order `fulfillmentStatus` transitions to `FULFILLED` in admin | Dispatched on `PATCH /api/admin/orders/[id]` and bulk fulfill | `Connected` |
